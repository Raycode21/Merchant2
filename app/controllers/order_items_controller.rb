class OrderItemsController < ApplicationController
  ...
  def create
 @order_item = @order.order_items.find_or_initialize_by(product_id: params[:product_id], order_id: @order.id)  #@order_item = OrderItem.new(product_id: params[:product_id], order_id: @order.id)
 #@order_item.quantity += 1
    respond_to do |format|
      if @order_item.save
        format.html { redirect_to @order, notice: 'Successfully added to cart!.' }
